...
const listFoods = (recipe) => {
  let result = [];
  let ingArr=['Gruffalo', 'oats', 'brown sugar', 'flour', 'pure maple syrup', 'chopped nuts', 'baking soda', 'baking powder', 'cinnamon', 'melted butter', 'fresh water'];
  for(let i=0;i<recipe.ingredients.length;i++){
    result.push(recipe.ingredients[i].slice(recipe.ingredients[i].indexOf(ingArr[i])));
  }
  return result ;
};
/* ------------------------------------------------------------------------------------------------
